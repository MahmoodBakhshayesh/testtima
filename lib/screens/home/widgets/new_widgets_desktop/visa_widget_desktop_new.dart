...
class VisasWidgetDesktop extends ConsumerWidget {
  const VisasWidgetDesktop({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final visas = ref.watch(visasProvider);
    if(visas.isEmpty) return SizedBox();
    return Container(
      padding: EdgeInsets.symmetric(vertical: 8, horizontal: 8),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadiusGeometry.circular(12)),
      child: Column(
        spacing: 8,
        children: visas.map((pass) {
          int index = visas.indexOf(pass);
          bool isLast = index == visas.length - 1;
...
class VisaWidgetDesktop extends ConsumerStatefulWidget {
  final DocumentDetail document;
  final int index;
  final bool isFirst;
  final bool isLast;

  const VisaWidgetDesktop({super.key, required this.document, required this.index, required this.isFirst, required this.isLast});

  @override
  ConsumerState<VisaWidgetDesktop> createState() => _VisaWidgetDesktopState();
}

class _VisaWidgetDesktopState extends ConsumerState<VisaWidgetDesktop> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      controller.addListener(() {
        Future(() {
          ref.read(visasProvider.notifier).updateAt(widget.index, widget.document.copyWith(documentNumber: controller.text));
        });
      });
    });
  }
  ...
  @override
  void didUpdateWidget(VisaWidgetDesktop oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    DocumentDetail d = ref.watch(visasProvider)[widget.index];
    final headerBg =null;
