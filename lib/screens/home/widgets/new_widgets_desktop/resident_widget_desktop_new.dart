...
class ResidentsWidgetDesktop extends ConsumerWidget {
  const ResidentsWidgetDesktop({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final residents = ref.watch(residentsProvider);
    if(residents.isEmpty) return SizedBox();
    ...
    return Container(
      ...
      child: Column(
        spacing: 8,
        children: residents.map((pass) {
          int index = residents.indexOf(pass);
          bool isLast = index == residents.length - 1;
...
class ResidentWidgetDesktop extends ConsumerStatefulWidget {
  final DocumentDetail document;
  final int index;
  final bool isFirst;
  final bool isLast;

  const ResidentWidgetDesktop({super.key, required this.document, required this.index, required this.isFirst, required this.isLast});

  @override
  ConsumerState<ResidentWidgetDesktop> createState() => _ResidentWidgetDesktopState();
}

class _ResidentWidgetDesktopState extends ConsumerState<ResidentWidgetDesktop> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      controller.addListener(() {
        Future(() {
          ref.read(residentsProvider.notifier).updateAt(widget.index, widget.document.copyWith(documentNumber: controller.text));
        });
      });
    });
  }
  ...
  @override
  void didUpdateWidget(ResidentWidgetDesktop oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    DocumentDetail d = ref.watch(residentsProvider)[widget.index];
    final headerBg =null;
            ...
            Row(
              ...
              children: [
                ...
                DotButton(
                  ...
                  onPressed: () async {
                    final confirm = await ConfirmOperation.getConfirm(
                      Operation(type: OperationType.warning, icon: ArtemisIcons.eraser_1, message: 'You are about to clear ${"Resident #${widget.index + 1}"}. Are you sure?', title: "Clear", actions: ["Cancel", "Confirm"]),
                    );
                    if (!confirm) return;
                ...
                ?widget.index == 0
                  ...
                  onPressed: () async {
                    final confirm = await ConfirmOperation.getConfirm(
                      Operation(type: OperationType.error, icon: ArtemisIcons.trash, message: 'You are about to delete ${"Resident #${widget.index + 1}"}. Are you sure?', title: "Delete", actions: ["Cancel", "Confirm"]),
                    );
                    if (!confirm) return;
