...
import '../home/home_view_phone.dart';
...
class ResultReportViewDesktop extends ConsumerStatefulWidget {
  static ResultReportController myResultReportController = getIt<ResultReportController>();

  const ResultReportViewDesktop({super.key});

  @override
  ConsumerState<ResultReportViewDesktop> createState() => _ResultReportViewDesktopState();
}

class _ResultReportViewDesktopState extends ConsumerState<ResultReportViewDesktop> {
                          ...
                          children: [
                            Expanded(
                              child: SingleChildScrollView(
                                ...
                                child: Column(
                                  children: [
                                    SizedBox(height: 0 + (resultMode ? additionalHeight : 0)),
...
//       : Row(
//     children: [
//       MyCountryFlagsPro.getFlag(a,width: 18,height: 14,borderRadius: BorderRadius.circular(2)),
//
//       const SizedBox(width: 2),
...
//       // height: 124 + (resultMode ? additionalHeight : 0),
//       child: Container(
//         padding: const EdgeInsets.only(top: 15, left: 8, right: 8, bottom: 16),
//         width: context.width,
...
//                                 Text(ref.watch(reportRefCodeShowProvider) ?? '', style: TextStyle(color: Colors.black,fontSize: 12)),
//                               ],
//                             ),
//                             Row(
//                               children: [
...
//                                 Text(ref.watch(reportCurrentStatusProvider)?.employeeId ?? '', style: TextStyle(color: Colors.black,fontSize: 12)),
//                               ],
//                             ),
//                             Row(
//                               children: [
...
class ResultReportAppBarDesktop extends StatelessWidget implements PreferredSizeWidget {
  static ResultReportController myResultReportController = getIt<ResultReportController>();
  static SmartOverlayMenuController controller = SmartOverlayMenuController();

  const ResultReportAppBarDesktop({super.key});
    ...
    return Container(
      ...
      child: SafeArea(
        child: Row(
          children: [
            BackButton(),
            Text("Result Report",style: TextStyle(fontSize: 20),),
            Expanded(
              child: Column(
