...
class OfflineScannerController extends ControllerInterface {
  ...
  Future<void> onFoundMrz(OcrMrzResult res) async {
    if (!res.valid.birthDateValid || !res.valid.expiryDateValid || !res.valid.docNumberValid || !res.valid.countryValid) {
      ...
      // log("${res.valid.docNumberValid} == ${res.documentNumber}");
    }
    // if (!scanning) {
    ...
    String short = res.documentCode.startsWith("P")
        ? "P"
        : res.documentCode.startsWith("V")
        ? "V"
        : "P";
  ...
  bool isSame(DocumentDetail? res, OcrMrzResult? res2) {
    ...
    final exact = res.documentExpiryDate?.format_yyMMdd == res2.expiryDate?.format_yyMMdd && res.birthDate?.format_yyMMdd == res2.birthDate?.format_yyMMdd && res.documentNumber == res2.documentNumber;
    if (exact == true) {
      return true;
    }
    // if(exact==false){
    // log("should isSameDocument");
    return isSameDocument(res, DocumentDetail.fromMrzResult(res2));
    // return false;
    // }
  }
  ...
  bool isSame2(DocumentDetail? res, DocumentDetail? res2) {
    ...
    final exact =
        res.documentExpiryDate?.format_yyMMdd == res2.documentExpiryDate?.format_yyMMdd &&
        res.birthDate?.format_yyMMdd == res2.birthDate?.format_yyMMdd &&
        res.documentNumber == res2.documentNumber &&
        res.documentIssueCountry?.code3 == res2.documentIssueCountry?.code3;
    // log("exact -- ${exact}");
    if (exact == true) {
      return true;
    }
    ...
    return res.documentCode == res2.documentCode &&
        res.documentExpiryDate?.format_yyMMdd == res2.documentExpiryDate?.format_yyMMdd &&
        res.birthDate?.format_yyMMdd == res2.birthDate?.format_yyMMdd &&
        res.documentNumber == res2.documentNumber;
  }
