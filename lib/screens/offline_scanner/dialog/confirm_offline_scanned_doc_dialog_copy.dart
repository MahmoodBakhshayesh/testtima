...
class ConfirmOfflineScannedDocDialog extends ConsumerStatefulWidget {
  final int timer;
  const ConfirmOfflineScannedDocDialog({super.key,this.timer = 5000});
...
class _ConfirmOfflineScannedDocDialogState extends ConsumerState<ConfirmOfflineScannedDocDialog> {
  ...
  void _restartTimer() {
    ...
    _autoCloseTimer = Timer( Duration(milliseconds: widget.timer), () {
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      child: Container(
        decoration: BoxDecoration(color:isExpiring?expiringColor.withOpacity(0.4): isExpired ? Colors.red.withOpacity(0.4) : null),
        child: Column(
